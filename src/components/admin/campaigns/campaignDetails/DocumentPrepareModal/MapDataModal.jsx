...
const MapDataModal = ({ isMapDataModal, setIsMapDataModal, campaignType }) => {
    ...
    const { loader, addLoader, getLoader, loginLoader, registerLoader } = useSelector((state) => state.loaders)
    ...
    return (
        <>
            ...
            ${saveButton ? "bg-green-600" : "bg-green-200 disabled"}
                rounded-md px-4 w-fit cursor-pointer
              text-white p-2 mx-2 font-semibold`} >
                        {addLoader ? <span className="text-md"><MoonLoader size={"22px"} /> &nbsp; Save</span> : "Save"}
                    </button>
